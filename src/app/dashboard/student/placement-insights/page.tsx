    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700 pb-12">
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                <Card className="border border-slate-200/60 dark:border-slate-800/60 shadow-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl overflow-hidden relative group hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
                    ...
                    <CardContent className="relative z-10 space-y-4">
                        ...
                        {analysis.tierSuggestions && analysis.tierSuggestions.length > 0 && (
                            <div className="pt-3 mt-3 border-t border-slate-100 dark:border-slate-800">
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Role & Strategy Guidelines</p>
                                <ul className="space-y-2">
                                    {analysis.tierSuggestions.map((sug, i) => (
                                        <li key={i} className="text-xs font-semibold text-slate-600 dark:text-slate-300 flex items-start gap-2">
                                            <span className="shrink-0 mt-0.5 rounded-full p-0.5 bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400">
                ...
                <Card className={cn(
                    ...
                    <CardContent className="relative z-10 space-y-4">
                        ...
                        {analysis.strategy.keyStrengths && analysis.strategy.keyStrengths.length > 0 && (
                            <div className="pt-3 mt-3 border-t border-slate-100 dark:border-slate-800">
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Key Profile Strengths</p>
                                <ul className="space-y-2">
                                    {analysis.strategy.keyStrengths.map((str, i) => (
                                        <li key={i} className="text-xs font-semibold text-slate-600 dark:text-slate-300 flex items-start gap-2">
                                            <span className="shrink-0 mt-0.5 rounded-full p-0.5 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400">
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <Card className="border border-slate-200/60 dark:border-slate-800/60 shadow-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl overflow-hidden relative group flex flex-col h-full">
                    <div className="absolute inset-x-0 -top-px h-px w-full bg-gradient-to-r from-transparent via-amber-500/50 to-transparent" />
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            <div className="p-1.5 bg-amber-100 dark:bg-amber-900/50 rounded-lg text-amber-600 dark:text-amber-400">
                        ...
                        <CardDescription>Personalized improvement areas generated from your profile</CardDescription>
                    </CardHeader>
                    <CardContent className="flex-1 flex flex-col justify-between">
                        {aiLoading ? (
                            <div className="flex flex-col items-center justify-center flex-1 py-12 gap-3">
                                <Loader2 className="h-7 w-7 animate-spin text-amber-500" />
                                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Analyzing your profile data...</p>
                            </div>
                            ...
                            <div className="flex flex-col items-center justify-center flex-1 py-10 text-center bg-emerald-50/30 dark:bg-emerald-950/20 rounded-xl border border-emerald-100 dark:border-emerald-900/50 border-dashed">
                                ...
                                <p className="text-sm text-emerald-700 dark:text-emerald-400 mt-1 max-w-xs">Your profile looks strong! Keep maintaining your current pace.</p>
                            </div>
                        )}
                    </CardContent>
                </Card>

                {/* Strategic 6-Week Recovery Roadmap */}
                <Card className="bg-gradient-to-br from-[#0c0a1f] to-indigo-950 text-white border-transparent shadow-[0_20px_50px_-15px_rgba(79,70,229,0.3)] overflow-hidden relative group flex flex-col h-full">
                    <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,rgba(99,102,241,0.15),transparent_60%)] pointer-events-none" />
                    <CardHeader className="relative z-10 border-b border-indigo-500/10 pb-5">
                        ...
                        <CardDescription className="text-indigo-200/70">Personalized plan to improve your weakest areas</CardDescription>
                    </CardHeader>
                    <CardContent className="relative z-10 pt-5 flex-1 flex flex-col justify-between">
                        {aiLoading ? (
                            <div className="flex flex-col items-center justify-center flex-1 py-12 gap-3">
                                <Loader2 className="h-7 w-7 animate-spin text-indigo-400" />
                                <p className="text-sm font-medium text-indigo-300">Generating your personalized roadmap...</p>
                            </div>
                        ) : aiRoadmap.length > 0 ? (
                            <div className="flex flex-col h-full">
                                <div className="space-y-0 text-sm">
                                    {aiRoadmap.map((phase, idx) => {
                                        const priorityColor = phase.priority === "Critical" ? "border-rose-500/30 text-rose-300 bg-rose-500/10" : phase.priority === "High" ? "border-amber-500/30 text-amber-300 bg-amber-500/10" : "border-emerald-500/30 text-emerald-300 bg-emerald-500/10";
                                        const lineColor = phase.priority === "Critical" ? "hover:border-rose-500/50" : phase.priority === "High" ? "hover:border-amber-500/50" : "hover:border-emerald-500/50";
                                        return (
                                            <div key={idx} className="relative pl-8 pb-6 last:pb-0">
                                                {idx !== aiRoadmap.length - 1 && (
                                                    <div className="absolute left-[11px] top-7 bottom-0 w-0.5 bg-indigo-900/50 group-hover:bg-indigo-500/50 transition-colors" />
                                                )}
                                                <div className="absolute left-0 top-1.5 h-6 w-6 rounded-full bg-slate-950 border border-indigo-500/30 flex items-center justify-center shadow-[0_0_10px_rgba(99,102,241,0.2)]">
                                                    <BookOpen className="w-3 h-3 text-indigo-400" />
                                                </div>
                                                <div className={cn("bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/5 transition-all hover:shadow-lg hover:-translate-y-0.5", lineColor)}>
                                                    <h4 className="font-bold text-white text-sm mb-1.5 flex flex-wrap items-center justify-between gap-2">
                                                        <span>{phase.week}</span>
                                                        <Badge variant="outline" className={cn("text-[9px] font-black uppercase tracking-widest px-2 py-0.5", priorityColor)}>{phase.priority}</Badge>
                                                    </h4>
                                                    <p className="text-xs font-medium text-indigo-300 mb-3 pb-2 border-b border-white/5">
                                                        <span className="text-indigo-400/50 uppercase tracking-widest text-[9px] font-black mr-2">Focus</span>{phase.focus}
                                                    </p>
                                                    <ul className="space-y-2">
                                                        {phase.tasks.map((task, tIdx) => (
                                                            <li key={tIdx} className="text-indigo-100/80 flex items-start gap-3 text-xs font-medium">
                                                                <div className="mt-1 h-3 w-3 rounded-full bg-indigo-500/20 border border-indigo-500/50 flex items-center justify-center shrink-0">
                                                                    <div className="h-1 w-1 bg-indigo-300 rounded-full" />
                                                                </div>
                                                                <span className="leading-snug pt-0.5">{task}</span>
                                                            </li>
                                ...
                                {/* Auto-Fill Gap Visual for short roadmaps */}
                                <div className="mt-auto pt-8">
                                    <div className="px-5 py-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md relative overflow-hidden group-hover:border-indigo-500/30 transition-colors">
                                        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/10 to-purple-500/10" />
                                        <div className="relative z-10 flex items-center justify-between">
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-300/70">Strategic Trajectory</p>
                                                <p className="text-xs font-bold text-indigo-100/90 leading-tight pr-4">Complete module milestones to accelerate your index rating</p>
                                            </div>
                                            <Target className="h-8 w-8 text-indigo-400/30 shrink-0" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center justify-center flex-1 py-12 gap-3">
                                ...
                                <p className="text-sm font-medium text-indigo-300">No recovery plan needed! Your profile is on track.</p>
                            </div>
                        )}
                    </CardContent>
                </Card>
